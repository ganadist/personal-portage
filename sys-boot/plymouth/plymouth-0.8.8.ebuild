...
src_prepare() {
	sed -i -e 's:SYSTEMD_UNIT_DIR=/lib/systemd/system:SYSTEMD_UNIT_DIR=/usr/lib/systemd/system:g' \
		configure.ac || die
	sed -i -e 's:/bin/udevadm:/usr/bin/udevadm:g' \
		systemd-units/plymouth-start.service.in || die
	sed -i -e 's:@PLYMOUTH_CLIENT_DIR@/plymouth quit:/bin/echo:g' \
		systemd-units/plymouth-quit.service.in || die
